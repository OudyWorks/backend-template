    ...
    static render(application, request, response, route, payload) {
        ...
        return this.renderer.renderToString(context).then(
            html => {
                ...
                return `<!doctype html><html data-vue-meta-server-rendered ${meta.htmlAttrs.text()}><head>${meta.meta.text()}${meta.title.text()}${meta.link.text()}${meta.style.text()}${meta.script.text()}${meta.noscript.text()}</head><body ${meta.bodyAttrs.text()}>${html}${meta.script.text({body: true})}</body></html>`
